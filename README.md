...
> 유저는 감정 일기(`emotion_diary`)를 기록하고,  
> 판매자(티처)는 감정 기반 콘텐츠(`contents`)를 등록합니다.

- 초기 설계에서는 `감정`이라는 키워드가 유저와 티처 모두에서 중복 사용되면서,  
  **감정 기준의 혼란 또는 키 충돌** 문제가 발생할 가능성이 있었습니다.
...
- 이 구조를 통해 감정 기준의 **일관성**을 확보하고,  
  **추천 알고리즘에서 동일한 감정 기준**을 적용할 수 있게 되었습니다.

---

### 2️⃣ 결제와 회원 테이블 간의 N:M 관계 해결

> 유저의 결제 흐름은 단순하지 않기 때문에,  
> 현실적인 결제 시나리오를 반영한 설계가 필요했습니다.

- 초기에 `cart`와 `payment`는 **1:1 관계**로 설계되어 있었습니다.  
- 그러나 유저가 **장바구니에서 일부만 선택해 결제**하거나,  
  **여러 번 나눠 결제하는 상황**에서는 한계가 발생했습니다.

- 따라서 `cart` 테이블은 그대로 유지하되,  
  `payment`와 `members` 사이에 `payment_detail`이라는 **junction 테이블**을 추가하여  
  **N:M 관계를 분리**하고 유연하게 구조를 개선했습니다.

- 이 구조는 향후 **부분 결제, 환불, 거래 내역 관리**에 있어 확장성과 무결성을 높여줍니다.

---

### 3️⃣ 사용자 보유 내역 `owned` 테이블 설계

> 결제 이력과 보유 상태는 분리되어야 한다는 관점에서 출발했습니다.
...
- 이를 해결하기 위해 `owned`라는 **보유 내역 전용 테이블**을 별도로 설계했습니다.  
  이 테이블은 `payment_detail`과 연결되어 있으며,  
  유저가 **현재 보유 중인 콘텐츠/아이템을 명확하게 관리**할 수 있습니다.

- 이 구조는 향후 **선물 기능, 사용 이력, 자동 만료 처리** 등 다양한 기능 확장에도 유리한 기반이 됩니다.
