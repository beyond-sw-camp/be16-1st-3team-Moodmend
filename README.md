...
- 초기 설계에서는 `감정`이라는 카테고리가 유저와 티처(판매자) 모두에서 중복 사용되면서,  
...
- 따라서 `cart` 테이블은 그대로 유지하되, `payment`와 `members` 사이에 <br> `payment_detail`이라는 junction 테이블을 추가하여 **N:M 관계를 분리**하고 유연하게 구조를 개선했습니다.
